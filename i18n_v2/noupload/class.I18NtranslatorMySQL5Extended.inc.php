...
/**
* extended translator class with MySQL 5 as a backend
* @author Michael Wimmer <flaimo@gmx.net>
* @category FLP
* @copyright Copyright © 2002-2005, Michael Wimmer
...
* @version 2.2
*/
class I18NtranslatorMySQL5Extended extends I18NtranslatorMySQL5 implements I18NtranslatorInterfaceExtended {

	/**
	* @param string $namespaces
	* @param object $locale I18Nlocale
	* @uses I18NtranslatorMySQL5::__construct()
	* @return void
	*/
	...
	* @uses I18NtranslatorMySQL5::$table
	* @uses I18NtranslatorMySQL5::getConn()
	...
	*/
	public function getLastUpdateDateNamespace($namespace = '') { // basically an overloaded method...
		...
		$sql  = 'CALL getLastUpdateDateNamespace ("' . addslashes($namespace) . '")';
	...
	* @uses I18Nbase::isFilledString()
	* @uses I18NtranslatorMySQL5::$translation_table
	...
	* @param object $locale I18Nlocale
	* @uses I18NtranslatorMySQL5::$database
	* @uses I18NtranslatorMySQL5::$table
	...
	* @uses I18NtranslatorMySQL5Extended::deleteLocaleString()
	* @uses I18NtranslatorMySQL5::getTranslatorLocale()
	*/
	...
	*/
	public function deleteTranslation($string = '', $namespace = 'lang_main') {
		$sql  = 'CALL deleteTranslation("' . addslashes(trim($string)) . '", "' . addslashes(trim($namespace)) . '")';
		$result = parent::getConn()->query($sql);
		if (parent::getConn()->affected_rows < 1) {
			return (boolean) FALSE;
		} // end if
	...
	*/
	public function addTranslation($string = '', $translation = '', $namespace = 'lang_main') {
		...
		$sql .= ' VALUES ("' . addslashes(trim($namespace)) . '", "' . addslashes(trim($string)) . '", UNIX_TIMESTAMP(), "' . addslashes(trim($translation)) . '")';
	...
	* @uses I18NtranslatorMySQL5::getConn()
	* @uses I18NtranslatorMySQL5Extended::stripTranslationTable()
	*/
	public function updateTranslation($string = '', $translation = '', $namespace = 'lang_main', $position = 1) {
		...
		if ($position == 0) { // changes string field
			$sql .= ' SET string="' . addslashes(trim($string)) . '", lastupdate=UNIX_TIMESTAMP()';
			$sql .= ' WHERE namespace="' . addslashes(trim($namespace)) . '" AND `' . $locale . '`="' . addslashes(trim($translation)) . '"';
		} elseif ($position == 2) { // changes translation field (default)
			$sql .= ' SET namespace="' . addslashes(trim($namespace)) . '", lastupdate=UNIX_TIMESTAMP()';
			$sql .= ' WHERE string="' . addslashes(trim($string)) . '" AND `' . $locale . '`="' . addslashes(trim($translation)) . '"';
		} else { // changes namespace
			$sql .= ' SET `' . $locale . '`="' . addslashes(trim($translation)) . '", lastupdate=UNIX_TIMESTAMP()';
			$sql .= ' WHERE string="' . addslashes(trim($string)) . '" AND namespace="' . addslashes(trim($namespace)) . '"';
		} // end if
