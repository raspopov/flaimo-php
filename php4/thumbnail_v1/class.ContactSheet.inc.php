...
class ContactSheet {
	...
	*/
	function returnBoxedThumb($thumbnail) {
		...
		$temp_image = @imagecreatetruecolor(($this->thumb_width + ($this->thumb_margin * 2)),
											($this->thumb_height + ($this->thumb_margin * 2)))
		...
		imagecopy($temp_image,
				  $thumbnail->returnThumbnail('png'),
				  $start_point,
				  $this->thumb_margin,
				  0,
				  0,
				  $thumbnail->getThumbWidth(),
		...
		if ($this->show_file_infos === TRUE) { // add file info
			$text_color = imagecolorallocate($temp_image,
											 ...
											 $this->text_color[1],
											 $this->text_color[2]);
			imagestring($temp_image,
						1,
						2,
						($this->thumb_height + ($this->thumb_margin * 2) - 20),
						$thumbnail->getPictureName(),
						$text_color);
			imagestring($temp_image,
						1,
						2,
						($this->thumb_height + ($this->thumb_margin * 2) - 10),
			...
			for ($y = 0; $y < $this->columns; $y++) {
				if (isset($this->thumbnails[$count])) {
					imagecopy($this->cs_image,
							  ...
							  0,
							  0,
							  $this->thumb_width + ($this->thumb_margin * 2),
							  $this->thumb_height + ($this->thumb_margin * 2));
