...
class Language {
	...
	* @since 1.000 - 2002/10/10
	*/
	function setLanguageFileName($name) {
		$this->languagefile_name = (string) ((strlen(trim($name)) > 0) ? $name : 'lang_main.inc');
	} // end function
		...
		if (!isset($this->languageFilesArray)) {
			...
			while ($lang_dir = @readdir($handle)) {
				if (is_dir($root . $lang_dir)) {
					if ($this->IsValidLanguageCode($lang_dir) == true &&
						file_exists($this->languagefile_path . '/' . $lang_dir . '/' . $this->languagefile_name)) {
	...
	*/
	function ChooseLang() { // PHP5: protected
		$this->loadUserClass();
		if (strlen(trim($this->inputlang)) > 0 &&
			$this->IsValidLanguageCode($this->inputlang) == TRUE) {
	...
	*/
	function ChooseCountry() { // PHP5: protected
		$this->loadUserClass();
		if (strlen(trim($this->inputcountry)) > 0 &&
			$this->IsValidLanguageCode($this->inputcountry) == TRUE) {
