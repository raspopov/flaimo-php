...
class AtomBuilder extends AtomBuilderBase {
	...
	protected function prepareAtomRequest($version = '1.0.0') {
		$this->setFilename(self::OUTPUT_FILENAME_PREFIX . $this->versions[$version] . '---' . $this->filename);
		if (strlen($this->getFilename() > 255)) {
			$this->setFilename(substr($this->getFilename(), 0, 255));
		} // end if
	...
	public function saveAtom($version = '1.0.0', $path = '', $filename = '') {
		$this->prepareAtomRequest($version);
		$this->setFilename($filename);
		$success = $this->version_objects[$version]->saveAtom($path);
		$this->setFilename();
		return $success;
	} // end function
